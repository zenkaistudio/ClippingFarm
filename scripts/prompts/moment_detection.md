...
- For each moment, write a punchy, scroll-stopping hook_title (under 60 characters) that would work as on-screen text — it should make sense on its own without watching the clip first. Wrap the two most important words or short phrases in `*asterisks*` so they can be rendered bold and highlighted on screen — one should be the subject or name, the other should be the action or the surprising part. Mark exactly two spans per title, keep each span short (1-3 words). Do not use emojis — they cannot be rendered in the caption overlay.
